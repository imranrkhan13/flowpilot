...
export default function CameraPanel({ scenarioId }: Props) {
  ...
  const handleFile = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    if (!file.type.startsWith('image/')) { setError('Please choose an image file.'); setResult(null); return; }
    if (file.size > 10 * 1024 * 1024) { setError('Please choose an image smaller than 10 MB.'); setResult(null); return; }
    ...
    reader.onloadend = async () => {
      ...
      } catch (cause) { setError(cause instanceof Error ? cause.message : 'We could not check this photo.'); }
  ...
  return (
    <section className="rounded-2xl border border-[#dfd2c5] bg-[#fffdf9] p-4 shadow-[0_10px_28px_rgba(85,55,37,0.06)]" aria-labelledby="camera-evidence-heading">
      <div className="flex items-start justify-between gap-3">
        <div><div className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#a35d3f]">Optional photo</div><h2 id="camera-evidence-heading" className="mt-1 text-sm font-extrabold text-[#3d281e]">Camera photo</h2></div>
        <span className="rounded-full bg-[#f1e2d4] px-2 py-1 text-[10px] font-bold text-[#8e5e43]">{scenarioId}</span>
      </div>
      <p className="mt-3 text-xs leading-5 text-[#806957]">Upload a place photo to estimate how many people are in one area. If AI is not connected, the app uses clearly marked sample data.</p>
      <div className="mt-4">
        <label className="mb-1.5 block text-[10px] font-bold uppercase tracking-[0.12em] text-[#a58b7a]" htmlFor="target-zone">Area to check</label>
      ...
      <label className="mt-3 block cursor-pointer rounded-xl border border-dashed border-[#cdb19c] bg-[#fffaf4] p-4 text-center text-xs font-bold text-[#8e5e43] transition hover:border-[#a35d3f] hover:bg-[#f8eee5]" htmlFor="camera-upload">
        {loading ? 'Checking photo…' : 'Choose a photo'}
      ...
      {result && (
        <div className="mt-4 space-y-3 rounded-xl border border-[#dfd2c5] bg-[#f8f1e9] p-3">
          ...
          <p className="text-xs leading-5 text-[#70594b]">{live ? 'Estimated with the connected AI model.' : 'Sample result; no live AI connection was used.'}</p>
          <div className="grid grid-cols-2 gap-2"><div className="rounded-xl bg-[#fffdf9] p-2.5"><div className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[#a58b7a]">People</div><div className="mt-1 text-lg font-extrabold text-[#4a2e22]">{result.peopleCount}</div></div><div className="rounded-xl bg-[#fffdf9] p-2.5"><div className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[#a58b7a]">How sure</div><div className="mt-1 text-lg font-extrabold text-[#4a2e22]">{Math.round(result.confidence * 100)}%</div></div></div>
          {result.timestamp && <div className="text-[10px] font-medium text-[#a58b7a]">Checked {new Date(result.timestamp).toLocaleTimeString()}</div>}
