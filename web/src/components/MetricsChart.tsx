...
export default function MetricsChart({ states, currentStep }: Props) {
  if (states.length < 2) return <div className="flex h-full items-center justify-center rounded-2xl border border-[#eadfd5] bg-[#fffaf4] text-xs font-semibold text-[#9b7962]">The chart will appear after the crowd starts moving.</div>;
  ...
  return <div className="h-full w-full rounded-2xl border border-[#eadfd5] bg-[#fffaf4] p-3"><div className="mb-1 flex items-center gap-4 text-[10px] font-semibold text-[#806957]"><span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-[#b97852]" /> Most crowded</span><span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-[#b14a3d]" /> Crowded spots</span><span className="ml-auto uppercase tracking-[0.14em] text-[#a58b7a]">Change over time</span></div><svg className="h-full w-full" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" role="img" aria-label="Chart showing crowding and crowded spots over time"><polyline points={densityPoints} fill="none" stroke="#b97852" strokeWidth="2" opacity="0.9" /><polyline points={bottleneckPoints} fill="none" stroke="#b14a3d" strokeWidth="2" opacity="0.9" strokeDasharray="4,3" /><line x1={currentX} y1={padding} x2={currentX} y2={height - padding} stroke="#8e5e43" strokeWidth="1.2" strokeDasharray="4,2" /></svg></div>;
}
