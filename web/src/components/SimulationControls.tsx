...
export default function SimulationControls({ isPlaying, speed, currentStep, totalSteps, onPlay, onPause, onReset, onSpeedChange }: Props) {
  ...
  return (
    <section className="rounded-2xl border border-[#dfd2c5] bg-[#fffdf9] p-3 shadow-[0_10px_28px_rgba(85,55,37,0.06)]" aria-label="Simulation controls">
      <div className="flex flex-wrap items-center gap-2.5">
        <button type="button" onClick={isPlaying ? onPause : onPlay} className="rounded-xl bg-[#4a2e22] px-4 py-2.5 text-sm font-extrabold text-white shadow-sm hover:bg-[#633d2d]">{isPlaying ? 'Pause' : 'Play simulation'}</button>
